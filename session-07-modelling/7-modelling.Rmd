...
```{r, echo= FALSE}
# your code here
formula_string <- paste("life_expectancy", "~ gdp + population + alcohol + gdp*population")
formula <- as.formula(formula_string)
reg3 <- lm(formula, data = life_expec_dat)
summary(reg3)

life_expec_dat1 <- life_expec_dat |> 
  mutate(
    sq_gdp = gdp^2,
    lg_pop = log(population)
  )

formula_string <- paste("life_expectancy", "~ gdp + population + alcohol + sq_gdp + lg_pop")
formula <- as.formula(formula_string)
reg4 <- lm(formula, data = life_expec_dat1)
summary(reg4)
