...
ggplot(df, aes(x = lifeExp)) +
  geom_density(fill = "lightblue", linewidth = 0.2) +
  labs( title = "Life Expectancy Density Plot", 
...
ggplot2::ggplot(penguins, aes(x = body_mass_g, color = species )) +
...
```{r}
# YOUR CODE HERE
ggplot(penguins, aes(x = penguins$species, y = penguins$body_mass_g )) +
  geom_boxplot() +
  geom_jitter(alpha = 0.2, color = "blue") +
...
p1 <- ggplot(df, aes(x = log(gdpPercap), y = lifeExp)) +
  geom_point(alpha = 0.4,
             size = 0.5) +
  labs(title = "Economic wealth and life expectancy",
       x = "ln GDP per capita",
...
p2 <- ggplot(df, aes(x = log(gdpPercap), y = lifeExp)) +
  geom_point(alpha = 0.4,
             size = 0.5) +
  labs(title = "Economic wealth and life expectancy",
       x = "ln GDP per capita",
...
# Grouped by species by adding color
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g, color = species)) +
  geom_point(alpha = 0.7,
             size = 0.5) +
  labs(title = "Flipper Length and Body Mass",
       ...
       y = "Body Mass (g)") +
  theme_light() + 
  geom_smooth(method = "lm", inherit.aes = F, aes(x = flipper_length_mm, y = body_mass_g)) # To overide the inheritance of aesthetics from the first part. 
```
